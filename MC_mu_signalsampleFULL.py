...
def muon_hists(events,etas,hists):
    ...
    #muon selection
    muon_quality_check = isHLTMatched(events, offlineMuons)
    ...
    cutHasOneMu = (ak.num(events["Muon_pt"], axis=1) == 1)
    events = events[cutHasOneMu]


    # Apply quality and eta cuts
    muon_quality_check = muon_quality_check[cutHasOneMu]
    eta_split = eta_split[cutHasOneMu]
    triggerSingleMuon = triggerSingleMuon[cutHasOneMu]

    # Combine cuts
    selected_muons = events["Muon_pt"][muon_quality_check & eta_split]

    # Apply trigger selection
    trigger_selected_muons = selected_muons[triggerSingleMuon]

    # Fill histograms
    for muons in selected_muons:
        for pt in muons:
            totalhist.Fill(pt)

    for muons in trigger_selected_muons:
        for pt in muons:
            filthist.Fill(pt)
